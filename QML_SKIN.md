...
- [`res/skins/LateNightQML`](res/skins/LateNightQML), an experimental, partial
  ...
  skin.
- [`res/skins/TouchQML`](res/skins/TouchQML), an experimental touch-first skin
...
When an agent works on `res/skins/TouchQML`, it must not configure, build, or
...
[`res/skins/TouchQML/main.qml`](res/skins/TouchQML/main.qml) is the beginning of
...
The implemented top stack uses one compact profile at every supported viewport,
with three rows at fixed Qt logical-pixel heights:
...
All rows receive the same `width / 2` split coordinate from the application
root. The left side occupies `0..splitX`, the right side occupies
...
- `DeckStatus`: track title/artist, loop and beat-jump size, sync/leader state,
  pitch, rate range, key, BPM, and remaining time. Loop and Beat Jump are flat
  48-pixel-high touch cells with original SVG icons and vertical dividers;
  Loop triggers `reloop_toggle`, while Beat Jump triggers `beatjump_forward`.
  Sync retains a 48-pixel-high touch target. A tap triggers the momentary
  `beatsync` control without latching Sync. A continuous two-second hold sets
  explicit-leader state through `sync_mode` when the partner deck has no
  leader; when the partner is already Lead, it instead enables `sync_enabled`
...
- `DeckOverview`: one clipped RGB full-track waveform per deck, with current
  position and cue/loop markers supplied by `Mixxx.Controls.WaveformOverview`.
- `LibraryView`: a touch-native all-tracks list backed by
...
application shutdown through `Qt.quit()`.
...
of the fixed 208-pixel stack. It belongs to a later, separate component. The
space below the current top stack is intentionally empty until subsequent
performance-view slices are implemented.
...
- TouchQML has a functional fixed-height navigation/deck-status/overview stack
  for two decks, centralized touch/theme metrics, controller-mappable view
  controls, live deck metadata and waveform-overview bindings, and a
  touch-native all-tracks browser with explicit load-to-deck actions.
...
- The example is fixed at four engine decks and 64 samplers, with a declared
  ...
  minimal skin.
- TouchQML is still an early slice: Browse switches the content area to a
  searchable all-tracks library, but library sources/playlists are not exposed
  yet, Touch FX still has no page, and transport, mixer, scrolling
  waveforms, performance pads, and the rest of the performance view are still
  absent.
...
- `res/skins/TouchQML/`: touch-first skin, currently the fixed three-row top
  stack.
...
- For TouchQML-only work, do not build or run Mixxx and do not add or modify
  tests or application source. Keep implementation changes inside
  `res/skins/TouchQML` and use static QML checks only.
